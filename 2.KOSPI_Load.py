...
# 배당금 버튼
def graph():
    if brandlabel2.cget("text") != "":
        index = list_of_brands.index(brandlabel2.cget("text"))

        baedang1 = int(Data[index*4+2][0].replace(',','')) # 배당금 1년 전
        baedang2 = int(Data[index*4+3][0].replace(',','')) # 배당금 2년 전
        baedang3 = int(Data[index*4+4][0].replace(',','')) # 배당금 3년 전

        baedangs = [baedang1, baedang2, baedang3]
        print(baedangs)
        print(baedang1, baedang2, baedang3)
        # Make a dataset
        plt.ticklabel_format(style='plain', axis='x', useOffset=False)
        plt.plot(range(2017,2020), baedangs)
        plt.gca().xaxis.set_major_locator(mticker.MultipleLocator(1))
        plt.xlabel("Current 3 Years")
        plt.ylabel("Dividend")
        plt.show()

    else:
        msgbox.showwarning("해당 사항 없음","찾는 기업(종목)명을 먼저 검색(입력)하세요. \n (알파벳 대/소문자 구분 필수)")
...
# Button for Graph
btn3 = Button(root, text="Current 3 years \n dividend price \n (Graph)", command=graph, width=13, height=3)
btn3.grid(row=5, column=5, pady=20, sticky=N)    
